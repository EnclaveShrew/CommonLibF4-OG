...
// scalar
template <class T, class Deleter>
class unique_ptr : public detail::unique_ptr<T, Deleter>
{
  private:
  ...
  public:
    ...
    unique_ptr(unique_ptr<U, E> &&a_rhs) noexcept //
        requires(std::convertible_to<typename unique_ptr<U, E>::pointer, pointer> &&
                 ...
                 (std::is_reference_v<deleter_type> ? std::is_nothrow_constructible_v<deleter_type, const E &> : std::is_nothrow_constructible_v<deleter_type, E &&>))
        : super(std::move(a_rhs))
    {
    }
    ...
    unique_ptr &operator=(unique_ptr &&a_rhs) noexcept //
        requires(((std::is_reference_v<deleter_type> ? std::is_nothrow_copy_assignable_v<std::remove_reference_t<deleter_type>> : std::is_nothrow_move_assignable_v<deleter_type>)) &&
...
// array
template <class T, class Deleter>
class unique_ptr<T[], Deleter> : public detail::unique_ptr<T[], Deleter>
{
  private:
    using super = detail::unique_ptr<T[], Deleter>;
    using super::_pointer;
  ...
  public:
    ...
    unique_ptr(unique_ptr<U, E> &&a_rhs) noexcept //
        requires((std::is_array_v<U> &&
                  ...
                  (std::is_reference_v<deleter_type> ? std::same_as<E, deleter_type> && std::is_nothrow_constructible_v<deleter_type, const E &> : std::convertible_to<E, deleter_type> && std::is_nothrow_constructible_v<deleter_type, E &&>)))
        : super(std::move(a_rhs))
    {
    }
