...
template <class T, class Allocator, class SizePolicy>
struct ArrayData : public ArrayDataBase<T, Allocator, SizePolicy> // 0
{
  public:
};
...
class Array : public ArrayBase<ArrayData<T, AllocatorGH<T, SID>, SizePolicy>> // 0
{
  public:
};

template <class T, std::int32_t STAT1 = 0, std::int32_t STAT2 = 0, class Alloc = AllocatorPagedCC<T, 0>>
class ArrayPagedBase : public Alloc
{
  public:
    // members
...
class ArrayPagedCC : public ArrayPagedBase<T, STAT1, STAT2, AllocatorPagedCC<T, STAT3>>
{
  public:
    T defaultValue; // 28
};
