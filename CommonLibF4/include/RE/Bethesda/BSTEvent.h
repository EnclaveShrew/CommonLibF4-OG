...
template <class T>
class BSTValueEventSink : public BSTEventSink<T> // 00
{
  public:
    ...
    virtual BSEventNotifyControl ProcessEvent([[maybe_unused]] const T &a_event, BSTEventSource<T> *) override
    {
...
template <class T>
class BSTValueEventSource : public BSTEventSink<BSTValueRequestEvent<T>> // 00
{
  public:
...
class BSTGlobalEvent : public BSTSingletonSDM<BSTGlobalEvent>
{
  public:
    ...
    class KillSDMEventSource : public BSTEventSource<KillSDMEvent>
    {
    };
    static_assert(sizeof(KillSDMEventSource) == 0x58);

    template <class Event>
    class EventSource : public BSTEventSink<KillSDMEvent>,
