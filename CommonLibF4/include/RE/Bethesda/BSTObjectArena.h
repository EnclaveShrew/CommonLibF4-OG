...
struct BSTObjectArenaScrapAlloc : private BSTObjectArenaScrapAllocBase // 0
{
  public:
    [[nodiscard]] void *allocate_bytes(std::size_t a_bytes)
    {
        return scrapHeap->Allocate(a_bytes, 0x8);
    }
...
template <
    ...
    std::uint32_t N = 32>
class BSTObjectArena : private Allocator // 00
{
  private:
  ...
  public:
    ...
    {
        return iterator{_head, _begin, _end};
    }
    ...
    {
        return iterator{_tail, _end, _end};
    }
    ...
    iterator push_back(const value_type &a_value)
    {
        return emplace(a_value);
    }
    iterator push_back(value_type &&a_value)
    {
        return emplace(std::move(a_value));
    }
  ...
  private:
        ...
        -> std::span<std::byte, sizeof(value_type)>
    {
        ...
        {
            const auto page =
                _free ? std::exchange(_free, _free->next) : // pull from free list
