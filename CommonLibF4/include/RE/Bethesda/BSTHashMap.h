...
class BSTScatterTable
{
  ...
  private:
    ...
    {
        if (const auto it = find(unwrap_key(a_value)); it != end())
        { // already exists
...
struct BSTScatterTableHeapAllocator
{
  public:
    ...
    [[nodiscard]] static constexpr size_type min_size() noexcept
    {
        return 1u << 3;
    }
    ...
    [[nodiscard]] void *get_entries() const noexcept
    {
        return _entries;
    }
