...
class __declspec(novtable) IMemoryStore : public IMemoryStoreBase // 0
{
  public:
...
class __declspec(novtable) IMemoryHeap : public IMemoryStore // 00
{
  public:
...
class __declspec(novtable) ScrapHeap : public IMemoryStore // 00
{
  public:
    ...
    struct FreeBlock : public Block // 00
    {
      public:
        // members
        FreeBlock *left;  // 10
    ...
    struct FreeTreeNode : public FreeBlock // 00
    {
      public:
        // members
    ...
    ~ScrapHeap() override
    {
        WinAPI::VirtualFree(baseAddress, 0, (WinAPI::MEM_RELEASE));
    } // 00

    // override (IMemoryStore)
    std::size_t Size(void const *a_mem) const override
    {
        return *static_cast<const std::size_t *>(a_mem) & ~(std::size_t{3} << 62);
    } // 01
    ...
    bool ContainsBlockImpl(const void *a_block) const override
    {
        return baseAddress <= a_block && a_block <= endAddress;
    } // 03
    void *AllocateAlignImpl(std::size_t a_size, std::uint32_t a_alignment) override
    {
        return Allocate(a_size, a_alignment);
    ...
    void DeallocateAlignImpl(void *&a_block) override
    {
        Deallocate(a_block), a_block = nullptr;
    } // 05
...
#define F4_HEAP_REDEFINE_HELPER(...)                                                        \
    ...
    {                                                                                       \
        ...
        {                                                                                   \
            stl::report_and_fail("out of memory"sv);                                        \
                                                                                            ...
                                                                                            \
    [[nodiscard]] void *operator new(std::size_t, void *a_ptr) noexcept                     \
    {                                                                                       \
        return a_ptr;                                                                       \
    }                                                                                       \
    [[nodiscard]] void *operator new[](std::size_t, void *a_ptr) noexcept                   \
    ...
    [[nodiscard]] void *operator new(std::size_t, std::align_val_t, void *a_ptr) noexcept   \
    {                                                                                       \
        return a_ptr;                                                                       \
    }                                                                                       \
    [[nodiscard]] void *operator new[](std::size_t, std::align_val_t, void *a_ptr) noexcept \
    ...
    {                                                                                       \
        return;                                                                             \
                                                                                            ...
                                                                                            \
    void operator delete(void *a_ptr, std::align_val_t)                                     \
    {                                                                                       \
    ...
    void operator delete(void *a_ptr, std::size_t, std::align_val_t)                        \
    {                                                                                       \
        RE::aligned_free(a_ptr);                                                            \
    }                                                                                       \
    void operator delete[](void *a_ptr, std::size_t, std::align_val_t)                      \
