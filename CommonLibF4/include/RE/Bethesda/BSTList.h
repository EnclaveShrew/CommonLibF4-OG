...
class BSSimpleList
{
  ...
  private:
    ...
    template <class U>
    class iterator_base : public boost::stl_interfaces::iterator_interface<
                              iterator_base<U>,
                              ...
                              U>
    {
      ...
      protected:
        ...
        {
            return iterator_base<std::remove_const_t<value_type>>{_proxy, _cur};
        }
  ...
  public:
    ...
    // 10)
    explicit BSSimpleList(std::initializer_list<value_type> a_init)
    {
    ...
    // 3)
    void assign(std::initializer_list<T> a_ilist)
    {
    ...
    {
        return mutable_reference().front();
    }
    ...
    {
        return mutable_reference().before_begin();
    }
    ...
    {
        return !empty() ? iterator{this, std::addressof(_root)} : end();
    }
    [[nodiscard]] const_iterator begin() const noexcept
    {
        mutable_reference().begin();
    }
    ...
    {
        return emplace_after(a_pos, a_value);
    }

    // 2)
    iterator insert_after(const_iterator a_pos, value_type &&a_value)
    {
        return emplace_after(a_pos, std::move(a_value));
    }
    ...
    {
        return erase_after(a_pos, ++a_pos);
    }
  ...
  private:
    ...
    [[nodiscard]] BSSimpleList<T> &mutable_reference() const noexcept
    {
        return const_cast<BSSimpleList<T> &>(*this);
    }
