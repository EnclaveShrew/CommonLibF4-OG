...
template <class T>
class __declspec(novtable) BSTCommonMessageQueue : public BSTMessageQueue<T> // 00
{
  public:
    ...
    // add
    virtual bool DoTryPush([[maybe_unused]] const T &a_message)
    {
        return false;
    } // 05
    virtual bool DoTryPop([[maybe_unused]] T &a_message)
    {
        return false;
    } // 06
...
template <class T>
class __declspec(novtable) BSTCommonLLMessageQueue : public BSTCommonMessageQueue<T> // 00
{
  public:
    // members
...
template <class T>
class __declspec(novtable) BSTCommonScrapHeapMessageQueue : public BSTCommonMessageQueue<T>
{
  public:
    // override
...
struct BSTLocklessQueue
{
  public:
    ...
    template <class T, std::uint32_t SIZE, std::uint32_t UNKNOWN>
    class alignas(0x80) ObjMultiProdCons : public ObjQueueBase<PtrMultiProdCons<T, SIZE * 2, UNKNOWN>, T, SIZE, UNKNOWN>
    {
    };
...
class __declspec(novtable) alignas(0x80) BSTCommonStaticMessageQueue : public BSTMessageQueue<T> // 00
{
  public:
