...
class DynamicMemoryManagementPol
{
  public:
    ...
    void deallocate(value_type *a_ptr, size_type)
    {
        free(a_ptr);
    }
};
...
class BSStringT : public Allocator<CharT, N> // 00
{
  public:
    using value_type = CharT;
    ...
    [[nodiscard]] operator std::basic_string_view<value_type, traits_type>() const noexcept
    {
        return {data(), size()};
    }
    ...
    {
        return _size == N ? static_cast<size_type>(traits_type::length(c_str())) : _size;
    }
