...
class __declspec(novtable) hkMemorySystem
{
  public:
    ...
    virtual hkResult GetHeapSoftLimit([[maybe_unused]] std::int32_t a_numBytes)
    {
        return {hkResultEnum::kFailure};
    } // 05
    virtual std::int32_t GetHeapSoftLimit() const
    {
        return -1;
    } // 06
    virtual bool SolverCanAllocSingleBlock([[maybe_unused]] std::int32_t a_numBytes)
    {
        return true;
    } // 07
    ...
    virtual void GarbageCollectThread([[maybe_unused]] hkMemoryRouter &a_router)
    {
        return;
    } // 0C
    ...
    virtual hkDebugMemorySystem *GetDebugInterface()
    {
        return nullptr;
    } // 10
    virtual hkResult GetMemorySnapshot(
        [[maybe_unused]] hkMemorySnapshot &a_snapshot) const
    {
        return {hkResultEnum::kFailure};
    } // 11
    virtual hkResult GetAllocationCallStack(
        ...
        [[maybe_unused]] std::size_t &a_allocSize)
    {
        return {hkResultEnum::kFailure};
    } // 12
    virtual void SetHeapScrubValues(
        [[maybe_unused]] std::uint32_t a_allocValue,
        [[maybe_unused]] std::uint32_t a_freeValue)
    {
        return;
    } // 13
    virtual std::uint32_t IsOk() const
    {
        return 1;
    } // 14
};
