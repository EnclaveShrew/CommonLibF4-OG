...
namespace RE
{
void *malloc(std::size_t a_bytes)
{
    return new std::byte[a_bytes];
}
void free(void *a_ptr)
{
    delete[] static_cast<std::byte *>(a_ptr);
}
    ...
    bool a_movable)
{
    ...
    for (char c = 'a'; c <= 'z'; ++c)
    {
        ...
        for (char d = 'a'; d < c; ++d)
        { // assert previous elements weren't lost
    ...
    // test ctor/assignment
    {
        ...
        if (a_movable)
        { // skip for non-copyable types, but assert the code still compiles
            auto t2 = t1;
            auto t3 = std::move(t2);
            REQUIRE(t2.empty());
            REQUIRE(eq(t3, t1));
...
[[nodiscard]] const key_type &get1(const key_type &a_value)
{
    return a_value;
}
[[nodiscard]] const key_type &get2(const RE::BSTTuple<const key_type, mapped_type> &a_value)
{
    return a_value.first;
}
[[nodiscard]] auto make1(key_type a_key, mapped_type)
{
    return a_key;
};
[[nodiscard]] auto make2(key_type a_key, mapped_type a_idx)
{
    return std::make_pair(std::move(a_key), a_idx);
};
