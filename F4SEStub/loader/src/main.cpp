...
class unique_struct
{
  ...
  private:
    ...
    void zero() noexcept
    {
        ZeroMemory(get(), sizeof(value_type));
    }
    ...
    std::chrono::milliseconds a_time)
{
    ...
    const ::DWORD time =
        a_time == std::chrono::milliseconds::max() ? INFINITE : static_cast<::DWORD>(a_time.count());
...
struct patch_t : Xbyak::CodeGenerator
{
...
class offset_calculator
{
  public:
    ...
    {
        return offset_of(type_t::kTotal);
    }
...
{
    auto exe =
        a_options.altexe ? *a_options.altexe : a_options.editor ? "CreationKit.exe"s
                                                                : "Fallout4.exe"s;
    auto dll = [&]() {
        if (a_options.altdll)
        {
        ...
        else
        {
            ...
            return fmt::format(
                "{}_{}_{}_{}.dll",
                (a_options.editor ? "f4se_editor"s : "f4se"s),
                version[0],
                version[1],
                version[2]);
        }
    ...
    std::string a_init)
{
    ...
    args.procName = !a_init.empty() ? reinterpret_cast<::LPCSTR>(mem.get() + calc.offset_of(type_t::kProcName)) : nullptr;
