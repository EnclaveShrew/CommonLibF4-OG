    ...
    else if (a_data.Container() && a_data.UniqueID())
    {
        ...
        if (const auto inv = cont->inventoryList; inv)
        {
            ...
            for (const auto &item : inv->data)
            {
                ...
                {
                    const auto xID =
                        stack->extra ? stack->extra->GetByType<RE::ExtraUniqueID>() : nullptr;
...
{
template <class T>
class BasicDelayFunctor : public RE::GameScript::DelayFunctor
{
  private:
  ...
  public:
    ...
    [[nodiscard]] bool IsLatent() const override
    {
        return false;
    }
    [[nodiscard]] FunctorType GetType() const override
    {
        return static_cast<FunctorType>(-1);
    }
    bool SaveImpl(RE::BSStorage &) const override
    {
        return false;
    }
    bool LoadImpl(const RE::BSStorage &, std::uint32_t, bool &) override
    {
        return false;
    }
