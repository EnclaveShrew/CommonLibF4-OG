    ...
    std::int32_t a_keycode)
{
    const auto find = [](std::int32_t a_button, RE::INPUT_DEVICE a_device) noexcept
        ...
        const auto context =
            controls ? controls->controlMaps[stl::to_underlying(RE::UserEvents::INPUT_CONTEXT_ID::kMainGameplay)] : nullptr;
        if (context)
        {
    ...
    RE::INPUT_DEVICE a_deviceType)
{
    ...
    const auto controls = RE::ControlMap::GetSingleton();
    const auto context =
        controls ? controls->controlMaps[stl::to_underlying(Context::kMainGameplay)] : nullptr;
    ...
    if (context)
    {
        const auto find = [&](RE::INPUT_DEVICE a_device) -> std::optional<std::int32_t> {
            ...
            return it != mappings.end() ? std::make_optional(it->inputKey) : std::nullopt;
        };
