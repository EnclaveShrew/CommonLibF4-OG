...
inline RE::TESObjectREFR *GetFurnitureReference(const RE::Actor &a_self)
{
    ...
    {
        const auto &furn =
            a_self.interactingState != RE::INTERACTING_STATE::kNotInteracting ? middleHigh->currentFurniture : middleHigh->occupiedFurniture;
        return furn.get().get();
    }
    else
    {
    ...
    RE::BIPED_OBJECT a_slotIndex)
{
    ...
    const auto object =
        a_self.biped ? a_self.biped->object[stl::to_underlying(a_slotIndex)].parent.object : // TODO: GetCurrentBiped()?
            nullptr;
